...
<form id="editthisclient" name="editthisclient" action="userview.php" method="post">
<input type="hidden" name="clientid" value="" />
</form>

<form id="attachmentsthisclient" name="attachmentsthisclient" action="userattachments.php" method="post">
<input type="hidden" name="clientid" value="" />
</form>

<form id="notesthisclient" name="notesthisclient" action="usernotes.php" method="post">
<input type="hidden" name="clientid" value="" />
</form>

<form id="accountsthisclient" name="accountsthisclient" action="useraccounts.php" method="post">
<input type="hidden" name="clientid" value="" />
</form>
...
echo "<TABLE width=90% class=table>";
	...
	for($i=0;$i < sizeof($cutstring);$i++){
		...
		if( preg_match("/^b=(.+)$/", $cutstring[$i], $matches) ){
			...
			//
			$query = sprintf("SELECT `displayfieldname` FROM `fields` WHERE `corefieldname` = '%s' LIMIT 1", $matches[1]);
...
while($clientdatarow = mysql_fetch_row($clientdata)){
	...
	// gotta make sure this user has permission to view this one, otherwise step to the next one
	//

	$query = mysql_fetch_assoc(	mysql_query(sprintf("SELECT `creator`,`usersecurity` from profile WHERE `clientid`='%s'", $clientdatarow[0])));
	
	if($query['creator'] != $_SESSION['username']){ continue; }
