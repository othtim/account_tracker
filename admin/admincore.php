...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if( isset($pdata['rebuildfields']) ){
		...
		$query = "SELECT displayfieldname, corefieldname, security, usersecurity from `fields`";
		...
		while($row = mysql_fetch_array($profilecolumndata)){
			...
			//we want to skip the special fields, namely "creator" and "usersecurity"
			if(( $row[0] == 'creator') or ($row[0] == 'usersecurity')) { continue; }

			echo $row[0] . "<BR>";
			
			////arrange an query. use escaped information, because we dont want taint in the DB.
			//
			$query = sprintf("INSERT INTO `fields` (displayfieldname, corefieldname, security, usersecurity) VALUES('%s', '%s', '%s', '%s')", mysql_real_escape_string($row[0]), mysql_real_escape_string($row[0]),0 ,0);	
...
while($fieldsrow = mysql_fetch_assoc($fieldstable)){

	echo "
	<table class=\"table\" width=\"60%\">
	<form action=\"admincore.php\" method=\"post\" name=".$fieldsrow['corefieldname'].">
	<tr>

	<td class=\"td1\" width=30%>
	" . $fieldsrow['corefieldname'] . "
	</td>

	<td class=\"td2\"width=30%>
	<input type=\"hidden\" value=\"" . $fieldsrow['corefieldname'] . "\" name=\"corefieldname\">
	<input type=\"text\" value=\"" . $fieldsrow['displayfieldname'] . "\" name=\"displayfieldname\">
	</td>

	<td class=\"td2\"width=5%>
	<input type=\"text\" value=\"" . $fieldsrow['security'] . "\" name=\"security\" size=5>
	</td>

	<td class=\"td2\"width=5%>";

		$query = sprintf("SELECT usersecurity FROM `fields` WHERE `corefieldname`='%s'",$fieldsrow['corefieldname']);
